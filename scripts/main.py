...
def fetch_news():
    ...
    url = ("https://push2.eastmoney.com/api/qt/slist/get"
           "?spt=1&fltt=2&invt=2&np=1&ut=bd1d9ddb04089700cf9c27f6f7426281"
           "&cb=&fields=f2,f3,f4,f12,f14&pn=1&pz=10&po=1&np=1"
           "&fid=f3&fs=b:BK0445")
    raw = fetch_url(url)
    # 换用新闻API
    news_url = "https://api3.cls.cn/merge"
    params = {"app": "CailianpressWeb", "os": "web", "sv": "8.4.6"}
    try:
        news_data = fetch_url(news_url + "?" + urllib.parse.urlencode(params))
        return [f"[数据源] 财联社新闻已获取，共{len(news_data)}字节"]
    except Exception:
        return []
...
分析维度：技术面、消息面、资金面、基本面"""
    ...
    user_message = f"""今天是{today}，请基于以下数据做盘前分析，选出3只今日值得关注的股票。
...
4. 输出格式必须严格遵循要求"""
    ...
    user_message = f"""今天是{today}，基于以下开盘数据，分析今日集合竞价阶段的异动股票，选出3只热度高、有潜力的。
...
def main():
    ...
    print(f"=== {today} AI 股票分析系统 - {args.mode} 模式 ===")
    ...
    if args.mode == "morning":
        ...
        analysis = morning_analysis()
    else:
        title = f"🔥 {today} 开盘异动追踪"
        analysis = opening_analysis()

    print(f"[步骤3] 分析完成，正在推送...")
    print(f"\n{'='*50}\n{analysis}\n{'='*50}")
    ...
    if success:
        ...
        print("[完成] ⚠️ 分析已完成（见上方），但微信推送失败")
        print("可能原因：PushPlus 未实名认证，请前往 verify.pushplus.plus 认证")
